...
from ess.nmx.configuration import TimeBinCoordinate, TimeBinUnit
...
def test_reduction_config() -> None:
    ...
    assert expected_config.inputs == config.inputs
    assert expected_config.workflow == config.workflow
    assert expected_config.output == config.output
